...
const endings = [
  ...
  ending(41002, '稳上 985', 'S', '你的实力和志愿都足够稳健。', '(HSCR>=630)&(VOL>=35)&(EVT![31028])', 90, 40, tags('985', '稳健')),
...
const scoreEndings = [
  ...
  [41106, '211 王牌专业', 'S', '(HSCR>=610)&(VOL>=65)', 98, 40],
  [41107, '双一流稳妥录取', 'S', '(HSCR>=600)&(VOL>=45)&(RSK<45)', 92, 35],
  ...
  [41111, '普通本科稳住', 'B', '(HSCR>=520)&(VOL>=25)&(RSK<60)', 68, 12],
  ...
  [41116, '状态崩盘出局', 'D', '(SPR<2)&(RSK>=70)', 96, -20],
  [41117, '高分低报遗憾', 'D', '(HSCR>=600)&(VOL<20)', 94, -15],
  [41118, '盲冲滑档', 'D', '(VOL<15)&(RSK>=50)', 93, -18],
  [41119, '调剂陌生专业', 'B', '(HSCR>=540)&(VOL<35)&(RSK<60)', 74, 8],
  [41120, '保底院校守住', 'B', '(HSCR>=510)&(VOL>=45)&(TLT?[21715])', 76, 14],
];
...
for (const [id, name, tier, condition] of talentEndings) {
  endings.push(ending(id, name, tier, `你的天赋组合把这局推向了「${name}」。`, condition, 88 + (id % 10), tier === 'D' ? -15 : 35, tags('天赋路线', tier)));
}

const eventEndings = [
  [41145, '阅读种子开花', 'S', '(EVT?[31101,31113,31125])&(HSCR>=590)'],
  [41146, '奥数早鸟', 'S', '(EVT?[31201,31213,31225])&(HSCR>=600)'],
  [41147, '英语领读生', 'A', '(EVT?[31202,31214,31226])&(HSCR>=560)'],
  [41148, '卷面分救命', 'B', '(EVT![31209])&(HSCR>=520)&(HSCR<560)'],
  [41149, '中考定位成功', 'A', '(EVT?[31308,31320,31332])&(VOL>=40)'],
  ...
  [41157, '补课精准提分', 'A', '(EVT?[31503,31515,31527])&(HSCR>=570)'],
  [41158, '平台期破局', 'S', '(EVT?[31505,31510,31019])&(HSCR>=590)'],
  [41159, '手机上交奇效', 'A', '(EVT?[31512,31524,31536])&(STR>=6)'],
  [41160, '一模敢想', 'S', '(EVT?[31601,31613,31625])&(SPR>=6)'],
  [41161, '二模阴影', 'C', '(EVT?[31608,31620,31632])&(SPR<5)'],
  ...
  [41169, '专业名误导', 'D', '(EVT?[31709,31721,31733])&(VOL<45)'],
...
for (const [id, name, tier, condition] of eventEndings) {
  endings.push(ending(id, name, tier, `关键经历串成了「${name}」的路线。`, condition, 83 + (id % 11), tier === 'D' ? -12 : tier === 'C' ? 5 : 30, tags('事件链', tier)));
}
