...
本文件用于记录《重回高三人生模拟》独立项目当前完成情况。后续 agent 开工前先读本文件，再读对应任务文档。
...
- 已按 `docs/11-simulation-optimization-plan.md` 完成一轮全量优化：模拟指标、录取线扩充、录取算法、录取 token、结局重平衡和属性成长复核均已落地。
- 后续重点：继续提高结局覆盖、微调 985/211 实录层级分布、降低 `INT/STR` 触顶率。
...
| 10 5000 局模拟优化 | `11-simulation-optimization-plan.md` | 部分完成 | A-F 已落地并通过 5000 局复测；结局覆盖和 985/211 实录层级仍需后续微调。 |
...
- Top 5 结局占比从 75.2% 降到 53.2%。
- 80 个结局中命中 45 个，结局覆盖仍不足。
- 录取 Top 2 院校占比从 69.6% 降到 17.4%。
- 滑档率从 0.1% 提高到 2.5%。
- `INT/STR` 触顶率仍高，后续还需继续处理。

待做：

- 继续提高结局覆盖，重点处理顶尖分数、隐藏路线和低分复读/专科路线。
- 微调 985/211 实录层级分布。
- 继续观察 `INT/STR` 触顶率。

验收：

- 5000 局结局覆盖当前 45/80，未达 >= 62/80。
- Top 1 结局 14.2%，Top 3 36.4%，Top 5 53.2%，已达标。
- B 档 6.0%，D 档 6.9%，已达标。
- 滑档率 2.5%，已达标。
- 录取 Top 1 院校 9.1%，Top 2 17.4%，已达标。
- 大档位分组满足 `06-qa-balancing.md` 目标。
